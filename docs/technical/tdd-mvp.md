...
6. **`sync` is a full, atomic rebuild.** No watermarks, no manifest, no incremental state. Parse every selected source, copy unselected vendor data into the new store in a temp directory, and swap the complete generation into place. A year of history parses in seconds; incrementality returns only if measurement says otherwise.
...
8. **Synthetic regression tests, local live contracts.** Invented records specify adapter behavior. Only explicitly selected, local-only live contract checks establish observed vendor compatibility; CI never reads live histories. No captured fixtures, derived JSON goldens, or redaction tooling are retained. See `docs/log-contracts.md`.
...
| `started_at` / `ended_at` | TIMESTAMPTZ¹ | first / last record timestamp |
...
¹ **Refined by #9 from this table's original `TIMESTAMP`.** With plain `TIMESTAMP`, `now() - INTERVAL 1 DAY` forces an implicit cast through the DuckDB session's local timezone, silently shifting every comparison by that offset — wrong answers for exactly the two README cookbook queries that filter on time. `TIMESTAMPTZ` keeps every comparison an unambiguous UTC instant regardless of the querying session's timezone. This only affects the DuckDB export's column types (`internal/adapters/duckdbcli/script.go`'s `read_json` `columns={...}` schema); the canonical store's on-disk JSON is unaffected (`time.Time` marshals to RFC 3339 either way).

## Vendor formats and mapping

Both formats are undocumented. The mappings below are our assumptions, exercised with invented records and checked against live examples only through the opt-in local contract tier. Unknown record types are counted and skipped.

### Claude Code (`~/.claude/projects/`)

*Use the local contract checker to establish which shapes are observed in your current logs.*
...
- Mapping: `tool_use` blocks in assistant messages → `tool_calls` rows (`id`, `name`, `input`); `tool_result` blocks in subsequent user-type records are joined back by `tool_use_id` to fill `output`/`status`, and do **not** produce `messages` rows; text blocks concatenate into `messages.text`. Additional mapping assumptions:
...
- Behavior of `internal/adapters/claudesource` (#6), covered by synthetic adapter tests:
  ...
  - **Session metadata is first-non-empty per field.** Initial bookkeeping may omit `cwd`, `gitBranch`, `version`, or `sessionId`. `project_name` is POSIX `path.Base(project_path)`.
...
- Layout: active `sessions/YYYY/MM/DD/rollout-*.jsonl` and flat
  `archived_sessions/rollout-*.jsonl`, beneath the Codex root.
- The first owning `session_meta.id` identifies a thread; `session_id` may
  identify its parent/root. Ignore inherited metadata when selecting identity.
- `response_item.message` maps user/assistant directly and developer to system;
  inter-agent `agent_message` is system context. Text blocks join with newlines;
  original source records remain in message `raw`. `turn_context` supplies model.
- Standalone function/custom calls each get an empty-text assistant carrier
  message. Function arguments decode as JSON when valid; freeform custom input
  is a JSON string. Outputs join by call ID; structured outputs stay JSON text.
  Missing output is pending; a returned output is ok unless explicitly failed.
  This status describes the invocation, not each nested command inside `exec`.
- A valid `subagent_history_start_ordinal` excludes inherited activity while
  allowing preceding model context to seed the child. Missing boundaries do not
  trigger guessed text-based deduplication. IDs include vendor, thread and entity
  kind. Codex's mapping stays inside its source adapter.
- Mirrored event notifications, reasoning and known bookkeeping are counted,
  not emitted again. Unknown types and malformed lines are skipped and counted.
  Skipped bookkeeping is not claimed to be retained in the canonical store.
- Assumptions and local verification: `docs/log-contracts.md`; synthetic examples: `internal/testing/testlogs/`.
...
- **The `ValidSessionDoc` pre-filter.** Before calling `Put`, `ingestVendor` skips — uncounted, never an error — any doc `ports.ValidSessionDoc` reports invalid. The log level distinguishes two cases behind that one guard: a doc that is invalid *and* carries no messages and no tool calls is a file that parsed to nothing at all, which is normal input and logs at **debug**; any other invalid doc logs at **warn**. A doc whose `Session.Vendor` disagrees with the `SourceRequest` that produced it is likewise skipped at warn — the store is keyed globally by session id, so a source stamping another vendor's namespace would corrupt the accounting. `ValidSessionDoc` is the exact predicate `jsonlstore.relPath` and `fakes.FakeStoreRebuild.Put` also call — see #7's note above for why a narrower hand-rolled check would be a gap, not a simplification.
- **Session accounting is keyed by session id, not by file, and the map is global to the run.** Two files that both parse to the same session id (three of the four pathological fixtures do) both reach `Put` — the later one wins in the store — but must only ever be counted once. `Run` owns a `map[string]sessionCounts` of session id → `(messages, toolCalls)` and passes it into every `ingestVendor` call, so dedup spans the whole run rather than one vendor; `VendorSummary.Sessions`/`Messages`/`ToolCalls` are derived from the entries that vendor contributed. Counting per-`Put`-call instead would print numbers that don't reconcile against what the store actually holds. **Reconciliation is exact for `Sessions`.** If a single id is stored by two *different* `SourceRequest`s in one run (not reachable from the CLI, which never emits a duplicate vendor), the id is credited to the request that stored it first, along with that request's counts — so `Messages`/`ToolCalls` can lag the store's final doc in that unreachable case.
...
- **Registry-based availability.** `Sync.Vendors()` supplies the sorted registered sources for CLI default fan-out, explicit selection, help and errors. Both Claude and Codex are registered; unknown/unavailable vendors fail before starting a rebuild. No fixed vendor switch is needed to register a future source.
...
- **Settled: vendor-scoped rebuilds.** `BeginRebuild(ctx, vendors)` preserves all unselected vendor subtrees byte-for-byte in staging, then accepts new docs only for selected vendors. Nil scope means full reset for direct callers; non-nil empty scope preserves all and permits no writes. `Sync.Run` passes its requested vendors, so missing selected sources remove their stale sessions while unselected/unregistered vendor data survives. Copy failures discard staging; a rejected out-of-scope write poisons commit. One whole-generation swap retains existing rollback guarantees; no independent subtree swaps or incremental metadata are introduced.

## Export path

`export duckdb` shells out to the `duckdb` CLI: a generated script builds the three relations from the store via `read_json`, applies the denormalization joins, and materializes them as `CREATE TABLE ... AS SELECT` directly into a fresh snapshot `.duckdb` file that `duckdb` opens as its database argument (written to a temp path, then renamed over the target). The export is a full snapshot each run, matching the full-rebuild sync.

**Settled by `internal/adapters/duckdbcli` (#9):**

- **Two script forms, chosen in Go.** `Script(hasDocs bool) string` (`script.go`) is a pure function of one bool — the store root is never a parameter and never appears in the generated SQL at all (see the next bullet). The populated form (`hasDocs=true`) builds a `_docs` temp table via `read_json('sessions/*/*.json', format='newline_delimited', maximum_object_size=64MiB, columns={...})` with an explicit `columns={...}` schema (never auto-detection — closes #7's flagged gap 2: each store file is a nested `session`/`messages`/`tool_calls` doc, and column auto-detection over that shape is unreliable, plus a field only `omitempty`'d out of every current record would silently vanish from auto-detected columns). The empty form (`hasDocs=false`) declares the same `_docs` shape with a bare `CREATE TEMP TABLE _docs (...)` and no `read_json` call at all, because `read_json` raises `IO Error: No files found that match the pattern` on a zero-match glob and no version-independent SQL workaround exists — so an empty store is branched **in Go**, not SQL. `storeHasDocs` (in `duckdbcli.go`) decides the branch: it `os.ReadDir`s `<root>/sessions` then each vendor subdirectory, returning true on the first `*.json` found. `sessions/` absent, or present but holding no `*.json` under any vendor subdirectory, is `false, nil` — a fresh or all-empty store is a legitimate empty export (closes #7's flagged gap 1), never an error. `<root>` itself not existing on disk at all *is* an error — `ErrNoStore`, naming `sync` as the fix, since that is the realistic cause (export run before any sync ever succeeded). Each form is pinned byte-for-byte against its own committed golden file — the populated form by `TestScriptPopulatedMatchesGolden` (`testdata/script_populated.golden.sql`), the empty form by `TestScriptEmptyMatchesGolden` (`testdata/script_empty.golden.sql`); neither test compares the two forms to each other.
- **Delivery: stdin, with `cmd.Dir` set to the store root — the store root is never a literal in SQL.** The generated script is piped to the `duckdb` process's stdin rather than passed via `-c` (a length ceiling, and a second quoting story) or a temp `.sql` file (`-f` support across versions is uncertain, and it's one more cleanup path); `cmd.Dir` is set to the store root, and the script's `read_json` glob (`'sessions/*/*.json'`) is relative. This is not just an escaping convenience: DuckDB glob-expands path arguments, so a store root containing `*`, `?`, or `[` would otherwise silently read the wrong files. It also makes `Script` a pure function of one bool, with no path parameter to keep in sync with the adapter's own path handling.
- **Temp *directory*, not temp file, then rename.** `os.MkdirTemp(filepath.Dir(req.Out), ".export-")`, with `defer os.RemoveAll(tmpDir)` registered on the line after `MkdirTemp` succeeds — before any other fallible call in `Export`. A temp *file* would be wrong: DuckDB writes a sibling `.wal` next to the database file, so a failed run with a temp file would leave a `logs.duckdb.tmp1234.wal` on disk; a temp directory can simply be removed whole regardless of what DuckDB wrote alongside the `.duckdb` file. The temp directory is created as a sibling of `req.Out` (same filesystem), so the final `os.Rename` onto `req.Out` is atomic; `req.Out` is never touched — no truncate, no create, no remove — until that one rename, so any previous export at that path survives byte-identically if anything upstream fails. The script's own `COMMIT; CHECKPOINT;` folds DuckDB's WAL into the database file before the process exits, so nothing is left for the rename to lose.
- **Missing-binary and failure diagnostics.** `exec.LookPath` resolves the binary before anything else (after the cheap, local `storeHasDocs` check — see above); `exec.ErrNotFound` is wrapped as `ErrBinaryNotFound` with an install hint naming duckdb.org and noting that only `export` needs the binary, `sync` never does. A different `LookPath` failure (found but not executable, say) gets a different message, since the fix is different. A non-zero `duckdb` exit names the binary, wraps the underlying `exec` error, and includes the last few lines of stderr. **No proactive version probe:** a second subprocess on every export would guard a failure that a bad-enough version already produces a precise parse/syntax error for; `MinVersion` (`"1.4"`, the version CI is actually pinned to and tested against) is documentation surfaced in error messages, not a floor `Export` checks itself or a tested contract across a version range. `-init /dev/null` neutralizes the user's `~/.duckdbrc` so it can never perturb the generated script.
- **The generated SQL is never exposed via a flag.** No `--print-script`: that would need either a third `Exporter` port method or a new core field for one debugging affordance, when `--log-level debug` (already the documented diagnostic channel — see Logging) can carry it instead. `a.log.Debug("duckdbcli: generated export script", "sql", script)` logs the full script on every export attempt at debug level; failure messages point there (`re-run with --log-level debug to print the generated SQL`).
- **CI needs a real `duckdb` binary for the integration tier — the first time #7's and #8's integration tests, and this ticket's, ever run in CI at all.** `requireDuckDB(t)` resolves `duckdb` via `exec.LookPath`; if absent, it skips **unless** `ALX_REQUIRE_DUCKDB=1` is set, in which case it fails the test instead. CI's native `gate-expensive` job sets `ALX_REQUIRE_DUCKDB=1` and installs a version-pinned `duckdb` release zip before running Go tests (`docs/development.md` and `.github/workflows/ci.yml`), so a duckdb-install problem in that job is unmistakably a test failure rather than a silently-skipped tier — that job is the load-bearing proof of this ticket's acceptance criterion. A second, additive CI job runs the same integration tier again inside a container built from `Dockerfile.duckdb`, with the tests themselves run `--network none`; it was never executed in this repository's authoring sandbox (no working container daemon there) and can be reverted independently of the native job if it proves flaky.
- **The README cookbook acceptance test rebases timestamps, not the query.** DuckDB has no clock override, so `TestCookbookQueries` builds a real store from the generated Claude examples via the real `sync` use case into `t.TempDir()`, then shifts every timestamp in every session doc under that temp store by one uniform `delta = time.Now().Add(-25*time.Hour).Sub(fixtureOldest)` (`fixtureOldest` being the earliest fixture timestamp) before exporting — touching only the temp store, never `internal/testing/testlogs/`. A uniform delta preserves every interval between records. The 25-hour anchor is deliberate: the fixture span is under 17 hours, so one fixture project lands *outside* the cookbook's `INTERVAL 1 DAY` window after rebasing while the other two land inside it — the time predicate is exercised in both directions, not merely satisfied or merely empty. Two of the three cookbook queries carry a literal that matches none of the fixtures (`project_name = 'fetch-context'`, `arguments LIKE '%git push%'`) — rather than edit the README (user-facing docs, now a tested contract, not test data) or invent a fixture to match it, each such query is asserted twice: **verbatim**, proving it binds, runs, and returns exactly 0 rows (catching a schema bug that a substituted query alone could hide); and **substituted**, with one documented `strings.Replace`, proving the actual data and the join-free denormalization. `TestCookbookQueriesMatchTheREADME` (unit tier, no `duckdb` needed) parses the ` ```sql ` fences out of `README.md` and fails the moment either copy drifts from the Go constants (`internal/adapters/duckdbcli/cookbook_query_test.go`) the acceptance test actually runs.
...
| `Exporter` | materialize the store into a sink | `duckdbcli` (`os/exec`) — landed #9 | recording exporter (`fakes.FakeExporter`) |
...
Same four-tier pyramid as `fetch-context` (see `fetch-context`'s own `docs/testing.md`, and this repo's `docs/testing.md` for how it's realized here), built on fakes rather than mocks (core decision 2):

- **Unit** (no tag): normalization from invented records, session-doc assembly, ingest-summary accounting — core use cases exercised against the fakes.
- **Integration** (`integration`): sync against generated synthetic trees on a real temp filesystem; export against a real `duckdb` binary when present.
...
- **E2E** (`e2e`): black-box runs of the compiled binary against synthetic examples under `AGENT_LOGS_EXTRACTOR_HOME`, mapped 1:1 to AC-IDs once `docs/acceptance.md` exists.

Invented examples are generated in temporary directories by `internal/testing/testlogs/`; adapter unit tests also construct small records inline. Fakes live in `internal/testing/fakes/`.

**Settled by #11:**

- **Contract checks have two layers.** `internal/testing/invariants` checks unified model structure and drift. `internal/testing/contracts` compares documented raw envelopes with normalized messages and tool joins, reporting only counts and fixed labels. Both engines are unit-tested with invented data. Live wrappers require explicit test-only vendor paths, skip when `CI` is set, and remain outside both gates. See `docs/log-contracts.md` for coverage limits.
- **`docs/acceptance.md` exists** with one `AC-*` ID per user-observable criterion, mapped 1:1 to a `tests/e2e/` test (or, for US-3/US-4's cookbook queries, to the existing `TestCookbookQueries`/`TestCookbookQueriesMatchTheREADME` — re-running those through the compiled binary would add subprocess cost without covering anything new; see `docs/acceptance.md` §12 R5). `tests/e2e/coverage_test.go`'s `TestACCoverage` enforces the mapping and is deliberately **untagged** so it runs inside `just gate` even though the e2e tier itself does not.
- **Exit codes are unchanged (0/1, not fetch-context's 0/1/2)** — recorded as a resolved decision (`docs/acceptance.md` §12 R1/R2) rather than silently left ambiguous; revisiting it is scoped as a separate ticket.
- **Container e2e reuses `Dockerfile.duckdb`**, overriding its `CMD` at run time (`just test-e2e-container`) rather than adding a second image — the same shape #9's `test-integration-container` already established.

## Open questions

1. **Claude Code sidechains: flatten into the parent session (settled by #6).** The mapping assumes:
...
2. Message-level project override: Claude records `cwd` per record, and a session's cwd can change mid-conversation. MVP: session-level `project_path` from the first record; flag drift in the contract tests if it occurs in practice. **Instrumented by #11's contract tier as observation `cwd_drift`** (`internal/testing/invariants.Observe`, reading `cwd` back out of `Message.Raw`): never fatal, reported as counts without logging any `cwd` values whenever a real `~/.claude` contract run finds more than one within a session — the early-warning signal this question asked for, not yet a resolution of it.
