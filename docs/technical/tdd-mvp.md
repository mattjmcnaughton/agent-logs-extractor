...
- **Filename mapping.** A session's file is `<vendor>/<stem>.json`, where `<stem>` is the session id with its exact `"<vendor>:"` prefix stripped, then every byte outside `[A-Za-z0-9._-]` percent-escaped (`%XX`, uppercase hex) into one path element. `Put` **rejects** (`jsonlstore.ErrInvalidSessionDoc`) a doc whose `Session.SessionID` does not carry that exact vendor prefix, or whose vendor/id is empty — it does not fall back to escaping the whole id. Escaping every byte, not just `:`, is load-bearing: mapping `:` → `_` is not injective (`claude:a_b` and `claude:a:b` would collide), and a session id that falls back to a source *filename stem* (e.g. `../../etc/passwd`) must never be able to write outside the store. An id whose escaped stem exceeds 200 bytes is truncated to 160 bytes plus a 16-hex-char sha256 suffix of the full id — still collision-resistant, no longer round-trippable, which is fine because nothing reads a session id back off a filename.
