...
- Settled by `internal/adapters/claudesource` (#6), the source adapter fixture-tested against every committed Claude fixture (`internal/adapters/claudesource/claudesource_test.go`, goldens under `testdata/`):
  ...
  - **Skip taxonomy:** the three reasons this document already names (`malformed_line`, `unknown_record_type`, `bookkeeping_record`) plus three adapter-local ones — `missing_message` (no `message` and, for `type: "system"`, no top-level `content` either), `missing_uuid` (no `uuid` to key a `messages` row on), `orphan_tool_result` (a `tool_result` block whose `tool_use_id` matches no earlier `tool_use`).
...
1. ~~Claude Code sidechains (subagent conversations, flagged by `isSidechain` and `tool_use.caller`): flatten into the parent session, or model as child sessions? MVP: flatten, keep `parentUuid` linkage in `parent_message_id`; revisit with real fixtures.~~ **Settled and closed by #6.** Real fixture evidence (`internal/testing/logfixture/claude/projects/-home-user-fixture-sidechain/`, ticket #5) confirms **flattening into the parent session** as the MVP approach:
   ...
   - **Merge ordering, also settled by #6:** flattened records — the parent transcript plus every `subagents/*.jsonl` transcript — are merged in **chronological order** by timestamp, tie-broken by `(fileRank, lineIndex)` with the parent transcript winning ties and an untimestamped record carrying its predecessor's order forward. This reproduces true reading order without depending on the parent link resolving, and generalizes to a `run_in_background` agent whose records interleave with everything the parent did while it ran — a "splice in right after the spawning message" rule would not.
