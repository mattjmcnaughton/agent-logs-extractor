...
- Settled by `internal/adapters/claudesource` (#6), the source adapter fixture-tested against every committed Claude fixture (the package's `*_test.go` files — `golden_test.go`, `parse_test.go`, `sidechain_test.go`, `sidechain_multi_test.go`, `content_test.go` — goldens under `testdata/`):
